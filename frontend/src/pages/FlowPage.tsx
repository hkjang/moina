...
function FlowContent({ mode }: { mode: FeedMode }) {
  ...
  return <div className="feed-page">
    ...
    <Modal open={compose} onOpenChange={(open) => !open && closeComposer()} title={quoteID ? 'Quote Moin' : '새 모인'} description="짧은 생각, 링크, 이미지 또는 영상을 나누세요." restoreFocusRef={composerTriggerRef}>{quoteQuery.loading ? <SkeletonFeed/> : quoteQuery.error ? <ErrorState message={quoteQuery.error} onRetry={quoteQuery.reload}/> : <MoinComposer quoteMoin={quoteMoin} onClearQuote={clearQuote} onCreated={() => { closeComposer(); void feed.reloadFirstPage(); }}/>}</Modal>
  </div>;
}
