...
MOINA `v0.1.26`은 Go modular monolith와 React SPA를 단일 binary/image로 배포합니다. 초기 제품에서 microservice 운영 복잡도를 만들지 않으면서 모듈 경계를 유지하고, 실제 부하가 확인되면 독립 worker나 search/notification service로 분리할 수 있게 합니다.
...
```text
moina:v0.1.26 (linux/amd64, distroless, non-root, read-only)
  ├─ /app/moina
  └─ /app/web/dist
