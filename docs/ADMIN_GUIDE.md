...
버전 `v0.1.30` · linux/amd64 단일 컨테이너 · 외부 PostgreSQL · 폐쇄망 운영

이 문서는 MOINA를 **설치하고 지키는 사람**을 위한 안내입니다. 화면을 쓰는 방법은
[사용자 가이드](USER_GUIDE.md)에 있으니 사용자에게는 그 문서를 안내하세요.

이 문서의 화면 캡처는 `v0.1.30` 서비스를 실제로 띄워 캡처 전용 데이터베이스에서 찍은
...
| 구성 | 내용 |
| --- | --- |
| 서비스 컨테이너 | `moina:v0.1.30` 하나. distroless·non-root·read-only로 실행하며 Go 서버와 React 웹 앱을 함께 담고 있습니다. |
...
GitHub Release에는 서비스 이미지 `moina-v0.1.30.tar.gz` 하나만 올라가고 SHA256 값은 릴리스
...
gzip -t moina-v0.1.30.tar.gz
gzip -dc moina-v0.1.30.tar.gz | docker image load
docker image inspect moina:v0.1.30 --format '{{.Os}}/{{.Architecture}} {{.Config.User}}'
```
...
태그(`moina:v0.1.30`)와 다르면 그 줄을 반입한 버전으로 맞춘 뒤 기동하세요. `pull_policy: never`
...
> 정보를 복구할 수 없습니다. `v0.1.30`은 온라인 root key 교체를 제공하지 않으므로 값을 바꾸지
> 마세요.
...
#### SSO 자동 로그인 (`autoLogin`)

Keycloak에 이미 로그인한 사용자가 MOINA를 열면 로그인 화면 없이 바로 본 화면으로 들어가게
하는 설정입니다. **기본값은 꺼짐**이라 켜기 전에는 아무것도 달라지지 않습니다. **Keycloak
로그인 사용**이 켜져 있어야 동작하며, 관리 API에서는 `PUT /api/v1/admin/oidc`의 `autoLogin`
필드입니다.

켜면 다음과 같이 동작합니다.

- 세션이 없는 브라우저가 앱 화면(`/flow`, `/moin/{id}` 같은 깊은 링크 포함)을 열면 웹 앱이
  `GET /api/v1/auth/oidc/login?prompt=none&returnTo=…`으로 **최상위 이동**합니다(숨은 iframe이
  아니므로 서드파티 쿠키가 막힌 브라우저에서도 동작합니다). Keycloak은 `prompt=none` 요청에
  화면을 그리지 않고, 세션이 있으면 곧바로 인가 코드를, 없으면 `error=login_required`를
  돌려줍니다.
- 세션이 있으면 평소 로그인과 같은 절차로 세션을 만들고 원래 열려던 자리로 돌아갑니다.
  `returnTo`는 `/`로 시작하고 `//`로 시작하지 않는 앱 내부 경로만 받습니다.
- 세션이 없으면 callback이 `/login?sso=none`으로 보내 로그인 화면을 보여 줍니다. 이것은
  실패가 아니라 평범한 답이므로 오류로 기록하지 않습니다.
- 사용자가 직접 **로그아웃**하면 그 탭에서는 다시 조용히 로그인시키지 않습니다. 다시
  로그인해 세션이 생기면 풀립니다.
- 시도는 **브라우저 탭 세션에 한 번**만 합니다(`sessionStorage`). 새 탭에서는 다시 시도하고,
  거절당한 뒤 새로고침하면 시도하지 않습니다. 사생활 보호 모드처럼 저장소를 읽지 못하면
  이미 시도한 것으로 쳐서 리다이렉트가 반복되지 않게 합니다.
- 로그인 화면(`/login`)과 API·MCP·상태 점검 경로에서는 시도하지 않습니다.
- 설정이 꺼져 있으면 주소에 `?prompt=none`을 붙여 불러도 서버가 평범한 로그인으로 바꿉니다.
  흐름을 바꿀 수 있는 자리는 이 설정 하나뿐입니다.

Keycloak 쪽 준비는 평소 로그인과 같습니다. Valid redirect URIs에 callback 주소가 등록돼 있고
연결 테스트가 통과하면 그대로 켤 수 있습니다. 성공한 자동 로그인은 감사 로그
`auth.oidc.login`에 `silent: true`로 남습니다.
...
사내 SMTP 릴레이로 이벤트 알림을 보냅니다. 기본값은 **꺼짐**이라 새로 설치한 곳에서는 아무
메일도 나가지 않고, 켜더라도 사내 릴레이에 흔한 **포트 25 · 인증 없음 · 자동 보안**이
기본값이며 인증과 암호화는 있으면 쓰는 선택 사항입니다. 사내 메일 서비스 `postra`를 릴레이로
가리키면 알림이 밖으로 나가지 않습니다.

| 설정 | 화면 항목 | 기본값 | 뜻 |
| --- | --- | --- | --- |
| `mail.enabled` | 이메일 알림 사용 | 꺼짐 | 관리자가 켭니다. 켜기 전에는 아무것도 보내지 않습니다. |
| `mail.smtp_host` | SMTP 서버 | — | 포트 없는 정확한 DNS 이름 |
| `mail.smtp_port` | 포트 | `25` | 사내 릴레이는 대개 25 |
| `mail.security` | 연결 보안 | `auto` | `auto`는 서버가 알리는 대로 — 465는 TLS, STARTTLS를 알리면 STARTTLS, 아니면 평문. `starttls`·`tls`·`none`은 고정. 인증 정보는 어떤 모드에서도 평문으로 보내지 않습니다. |
| `mail.skip_tls_verify` | TLS 인증서 검증 건너뛰기 | 꺼짐 | 사내 인증서가 사설 CA일 때만. 암호화는 유지합니다. |
| `mail.username` · `mail.password` | 사용자 이름 · 비밀번호 | 빈 값 | 인증 없는 릴레이가 흔하므로 선택 사항. 비밀번호는 암호화되어 조회 응답·화면·감사 로그에 원문이 나오지 않고 "설정됨"만 보입니다. |
| `mail.from_address` · `mail.from_name` | 보내는 이메일 · 보내는 이름 | — · `MOINA` | 보내는 사람 |
| `mail.base_url` | 일반 설정의 **사이트 기본 주소** | — | 메일 속 링크가 가리킬 이 앱의 주소. 따로 두지 않고 일반 설정을 씁니다. |
| `mail.timeout_seconds` | 연결 제한 시간(초) | `10` | 3~60 |
| `mail.notify_<이벤트>` | 메일로 보내는 이벤트 | 켜짐 | 이벤트 종류별 스위치. 끄면 그 종류만 멎고 알림 센터에는 그대로 남습니다. |

1. 서버 DNS 이름과 port를 나눠 입력합니다. 사내 릴레이라면 포트 25와 자동 보안을 그대로 둡니다.
2. 인증이 필요한 릴레이만 사용자 이름·password를 입력합니다. `auto`에서 릴레이가 STARTTLS를
   알리지 않으면 인증 정보를 평문으로 보내지 않고 실패로 기록하므로, 그런 릴레이는 인증 없이
   쓰거나 STARTTLS를 켜야 합니다.
3. 보내는 이메일·이름을 입력합니다.
4. DNS가 사설 주소를 반환하는 내부 서버만 사설망 SMTP 허용을 켭니다. 입력한 정확한
   `host:port` 하나만 열립니다.
5. **저장 후 테스트 메일**로 현재 관리자 프로필 이메일에 실제 한 통을 보내고 결과를 그 자리에서
   봅니다. 릴레이 설정은 한 번에 맞는 일이 드뭅니다 — 실패하면 오류 문구가 토스트와 **발송
   기록**에 함께 남습니다.
6. **이메일 알림 사용**을 켜면 사용자가 개인 알림 설정에서 이메일 채널을 고를 수 있습니다.
   받을지 말지는 받는 사람이 정합니다.

**메일로 보내는 이벤트**는 이 앱에서 사람이 실제로 기다리는 일만 고른 것입니다 — 이 메일이
오지 않으면 누군가 손해를 보거나 화면을 계속 새로고침하는 것. 단순한 "무언가 바뀜"(Signal·
Link·Quote·Remoin)은 메일이 되지 않고 알림 센터와 요약에만 남습니다.

| 이벤트 | 왜 메일인가 |
| --- | --- |
| 승인 요청 `approval_requested` | 승인자가 움직여야 요청자의 Moin이 게시됩니다. |
| 승인 결과 `approval_decided` | 요청자가 승인·반려 결과를 기다립니다. |
| 멘션 `mention` | 누군가 나를 직접 불러 답을 기다립니다. |
| 내 Moin의 Echo `echo` | 내 Moin에 달린 답글 — 대화가 나를 기다립니다. |
| 요약 `digest` | 사용자가 켠 주기로 활동을 묶어 한 통으로 보냅니다. |
| 계정 보안 `security` | 사용자가 끌 수 없는 운영 기록입니다. |

시끄러우면 사람들은 규칙을 만들어 통째로 버리므로, 자기가 한 일은 자기에게 보내지 않고 내
Moin의 Echo에서 나온 멘션은 Echo 메일 한 통으로 묶습니다.

메일은 별도 `notification.email` Outbox event로 **배경에서** 보내므로 릴레이가 느리거나 죽어
있어도 Moin 게시·Echo·승인 요청은 평소처럼 끝납니다. 실패는 지수 backoff 뒤 Dead Letter에
남고, 원인을 해결한 다음 감사 로그의 **실패 이벤트 복구**에서 재처리합니다.

**발송 기록**은 시도마다 남습니다 — 언제, 어떤 이벤트로, 누구에게, 어떤 제목이, 되었는지.
성공과 실패가 모두 있어야 "안 왔다"는 문의에 답할 수 있습니다. 본문은 담지 않습니다(담으면
기록이 그 자체로 유출 경로가 됩니다). 같은 메일의 재시도는 새 줄이 아니라 시도 횟수를 올리며,
기록은 알림 보존 기간(3.3절 `notificationDays`)과 같이 정리됩니다. 화면은 메일이 켜져 있거나
기록이 하나라도 있을 때 SMTP 메일 설정 아래에 보이고, `GET /api/v1/admin/smtp/deliveries`로도
읽을 수 있습니다.
...
바꾸거나 게시물을 지웁니다. `v0.1.30`은 신고 유형·우선순위·SLA를 사용자 정의하는 설정을
...
- 같은 시점의 `MOINA_ENCRYPTION_KEY`와 현재 이미지 `tar.gz`를 **서로 다른 보안 영역**에
  보관합니다.
- 복구 훈련에서 미디어·DB backup·원래 key가 함께 유효한지 확인합니다. `v0.1.30`은 온라인
