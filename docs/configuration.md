...
이 문서는 `v0.1.8`의 환경변수 계약과 관리자 화면에서 변경할 수 있는 설정을 구분합니다.
...
관리자 공통 system instruction, temperature와 사용자별 사용량 정책은 `v0.1.8` 설정 항목이 아닙니다. 필요한 경우 upstream AI gateway에서 적용하고, MOINA에 설정된 상한보다 더 좁은 한도로 운영합니다.
...
신고 유형·제재 단계·보존 기간을 관리자가 사용자 정의하는 정책 모델은 `v0.1.8`에 없습니다. 기관별 보존 규정은 PostgreSQL 백업·삭제 절차와 운영 문서로 관리합니다.
...
`VERSION`, Go binary, React asset, `/api/v1/version`, 로그인 화면, 프로필 메뉴와 OCI label의 값은 모두 `v0.1.8`로 일치해야 합니다. 런타임 환경변수로 버전을 덮어쓰지 않습니다.
