...
- 신뢰 Proxy IP/CIDR 기반 실제 Client IP 계산, 반복된 전달 header 줄의 단일 chain 결합, 감사 Proxy Chain과 다중 인스턴스 공용 PostgreSQL 요청 한도
...
- pg_notify로 즉시 전파되는 설정·역할 권한 캐시와 분당 1회로 제한한 API 키 사용 기록
- 검색어가 없는 추천 조회의 전용 질의, 문서와 일치하는 검색 `offset` 페이지네이션
- React·Radix·아이콘 분리 chunk와 상한이 있는 브라우저 API 캐시
- CI에서 실행하는 staticcheck·ESLint 정적 분석과 govulncheck·npm audit 의존성 검사
- 화면 밖 Flow 카드의 style·layout을 건너뛰는 렌더링과 캡처 도구 전용 해제
- pool 예산이 있을 때만 병렬로, 없으면 순차로 낮아지는 `type=all` 검색
...
`make lint`는 Go staticcheck와 프런트엔드 ESLint를, `make audit`은 실제로 호출되는 Go 취약점(govulncheck)과 npm 취약점을 검사합니다. ESLint는 도입 시점의 경고 40건을 예산으로 고정하므로 기존 경고를 고칠 수는 있어도 새 경고가 늘면 실패합니다.
...
```text
dist/moina-v0.1.38.tar.gz
dist/moina-v0.1.38.tar.gz.sha256
```

`.sha256`은 로컬 반입 검증용입니다. GitHub Release에는 사용자 요구에 따라 서비스 이미지 `moina-v0.1.38.tar.gz` 하나만 올리고 SHA256 값은 릴리스 본문에 기록합니다.
...
```bash
sha256sum moina-v0.1.38.tar.gz
gzip -dc moina-v0.1.38.tar.gz | docker image load
docker image inspect moina:v0.1.38
...
git tag -a v0.1.38 -m "moina v0.1.38"
...
```text
image: moina:v버전          예: moina:v0.1.38
file:  moina-v버전.tar.gz  예: moina-v0.1.38.tar.gz
```
