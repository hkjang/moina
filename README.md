...
현재 서비스 버전은 `v0.1.19`입니다. 로그인 화면과 프로필 컨텍스트 메뉴에서도 같은 버전을 확인할 수 있습니다.

`v0.1.19`는 Moin 본문에 적은 http·https 주소가 평문으로만 남아, 누르면 해당 사이트 대신 Moin 상세 화면이 열려 읽는 사람이 주소를 직접 복사해야 하던 문제를 고칩니다. 이제 주소를 하나의 token으로 인식해 링크로 표시하고 `target="_blank"`와 `rel="noopener noreferrer nofollow"`를 붙이며, 경로 안의 `@`·`#`가 멘션·토픽으로 떨어져 나가지 않습니다. 인식 문자는 RFC 3986 ASCII 집합으로 제한해 주소 뒤에 붙여 쓴 한글 조사가 링크로 빨려 들어가지 않고, 문장 끝 구두점은 본문으로 되돌리되 주소가 연 괄호는 링크에 유지합니다. `v0.1.18`의 수정됨 표시와 정확한 시각 tooltip, `v0.1.17`의 상대 시각 연도 표기, `v0.1.16`의 반복된 전달 header 줄 단일 chain 결합, `v0.1.15`의 staticcheck·ESLint 정적 분석과 govulncheck·npm audit 의존성 검사, `v0.1.14`의 설정·권한 캐시와 검색 개선은 그대로 유지합니다. `v0.1.12`의 `Ctrl/⌘+K` 전역 빠른 이동 팔레트, 키보드 결과 탐색, 최근 방문 복귀와 `G` 연속 화면 단축키는 그대로 제공합니다. 화면·설정·관리 메뉴는 현재 권한에 맞게 노출되고, 입력한 문장은 통합 검색으로 바로 이어집니다. `C`를 누르면 입력 중이거나 다른 Dialog를 사용 중이지 않을 때 새 Moin 작성을 즉시 시작합니다.
...
```text
dist/moina-v0.1.19.tar.gz
dist/moina-v0.1.19.tar.gz.sha256
```

`.sha256`은 로컬 반입 검증용입니다. GitHub Release에는 사용자 요구에 따라 서비스 이미지 `moina-v0.1.19.tar.gz` 하나만 올리고 SHA256 값은 릴리스 본문에 기록합니다.
...
```bash
sha256sum moina-v0.1.19.tar.gz
gzip -dc moina-v0.1.19.tar.gz | docker image load
docker image inspect moina:v0.1.19
...
git tag -a v0.1.19 -m "moina v0.1.19"
...
```text
image: moina:v버전          예: moina:v0.1.19
file:  moina-v버전.tar.gz  예: moina-v0.1.19.tar.gz
```
